...
export default function AdminDashboard() {
  ...
  // Injeção da Identidade Visual via CSS no topo do painel
  const brandStyles = `
      ...
      <div className="brand-theme min-h-screen flex flex-col md:flex-row antialiased text-[14px]">
        ...
        <div className="w-full md:w-[220px] shrink-0 bg-[var(--leather-2)] border-b md:border-b-0 md:border-r border-[var(--line)] p-4 md:p-5 flex flex-row md:flex-col justify-between md:justify-start items-center md:items-stretch z-10 sticky top-0 md:h-screen">
          ...
          <div className="flex flex-row md:flex-col gap-2">
            <button onClick={() => setAbaAtiva('agenda')} className={`px-3 py-2 md:py-2.5 rounded-md text-[13px] font-medium transition-all ${abaAtiva === 'agenda' ? 'bg-[rgba(201,162,75,0.10)] text-[var(--brass-bright)] border border-[rgba(201,162,75,0.25)]' : 'text-[var(--paper-dim)] hover:bg-[var(--leather-3)]'}`}>
              Agenda
            </button>
            <button onClick={() => setAbaAtiva('financeiro')} className={`px-3 py-2 md:py-2.5 rounded-md text-[13px] font-medium transition-all ${abaAtiva === 'financeiro' ? 'bg-[rgba(201,162,75,0.10)] text-[var(--brass-bright)] border border-[rgba(201,162,75,0.25)]' : 'text-[var(--paper-dim)] hover:bg-[var(--leather-3)]'}`}>
              Financeiro
            </button>
          </div>
        ...
        <div className="flex-1 p-5 md:p-9 pb-24 overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start mb-8 gap-4">
            ...
            <div className="w-full sm:w-auto text-right">
              <button 
                onClick={() => abaAtiva === 'agenda' ? setModalAgendamento(true) : setModalTransacao(true)} 
                className="w-full sm:w-auto font-semibold text-[12.5px] px-4 py-[9px] rounded-[5px] border-none bg-[var(--brass)] text-[var(--leather)] cursor-pointer hover:bg-[var(--brass-bright)] transition-colors shadow-lg"
              >
                    ...
                    {agendamentos.length === 0 ? (
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full border-collapse min-w-[500px]">
                          <thead>
                            <tr>
                              <th className="text-left font-mono text-[10px] uppercase tracking-[.06em] text-[var(--paper-dim)] font-normal pb-3 border-b border-[var(--line)] pl-2">Hora / Dia</th>
                              ...
                              <th className="text-left font-mono text-[10px] uppercase tracking-[.06em] text-[var(--paper-dim)] font-normal pb-3 border-b border-[var(--line)]">Serviço</th>
                              <th className="text-right font-mono text-[10px] uppercase tracking-[.06em] text-[var(--paper-dim)] font-normal pb-3 border-b border-[var(--line)] pr-2">Status</th>
                            </tr>
                          </thead>
                          <tbody>
                            {agendamentos.map(ag => {
                              ...
                              return (
                                <tr key={ag.id} className={`${isCancelado ? 'opacity-40' : ''}`}>
                                  <td className="py-3 px-2 border-b border-[var(--line)] font-mono text-[var(--copper-bright)] text-[13px]">
                                    {hora} <span className="text-[10px] text-[var(--paper-dim)] ml-1">{dataCurta}</span>
                                  </td>
                                  <td className="py-3 border-b border-[var(--line)] font-semibold text-[13px] text-[var(--paper)]">
                                    {ag.clientes?.nome}
                                    <div className="text-[10px] text-[var(--paper-dim)] font-mono mt-0.5">{ag.clientes?.telefone}</div>
                                  </td>
                                  <td className="py-3 border-b border-[var(--line)] text-[12.5px] text-[var(--paper-dim)]">
                                    {ag.servicos?.nome}
                                  </td>
                                  <td className="py-3 px-2 border-b border-[var(--line)] text-right">
                                    {isCancelado ? (
                                      <span className="text-[10px] font-bold py-1 px-[9px] rounded-full uppercase tracking-[.03em] bg-[rgba(239,230,216,0.06)] text-[var(--paper-dim)] border border-[var(--line)]">Cancelado</span>
                                    ) : (
                                      <span className="text-[10px] font-bold py-1 px-[9px] rounded-full uppercase tracking-[.03em] bg-[rgba(201,162,75,0.15)] text-[var(--brass-bright)] border border-[rgba(201,162,75,0.35)]">Confirmado</span>
                                    )}
                                  </td>
                                </tr>
                              );
